...
async function downloadFileWithResume(url, destination, expectedMd5, size, fileId, retryCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.get(url, requestOptions, response => {
      ...
      const totalBytes =
        parseInt(response.headers['content-range']?.split('/').pop() || response.headers['content-length'], 10) ||
        size;
      ...
      response.on('data', chunk => {
        ...
        if (SPEED_LIMIT_BYTES > 0) {
          ...
          if (currentSpeed > SPEED_LIMIT_BYTES) {
            ...
            setTimeout(
              () => req.resume(),
              Math.ceil(((downloadedBytes - lastByteCount) / SPEED_LIMIT_BYTES) * 100),
            );
          }
...
ipcMain.handle('open-logs', async () => {
  ...
  if (!fs.existsSync(logFileFull))
    fs.writeFileSync(logFileFull, `SWG Returns Launcher Log\nCreated: ${new Date().toISOString()}\n\n`);
...
process.on('uncaughtException', error => {
  try {
    fs.appendFileSync(logFile, `${new Date().toISOString()} - Uncaught Exception: ${error.stack}\n`);
  } catch (_) {}
});
